...
print("Setze Infowebinar-Daten für Lehrer ID 501-1000 basierend auf Schul-Förderung...")
...
# 1. Lehrer ID 501-1000 mit ihren Schulen abrufen
cursor.execute("""
    SELECT t.id, s.foerderung 
    FROM teachers t
    LEFT JOIN schools s ON t.school_id = s.id
    WHERE t.id BETWEEN 501 AND 1000
    ORDER BY t.id
""")
teachers_data = cursor.fetchall()

print(f"Verarbeite {len(teachers_data)} Lehrer (ID 501-1000)...")
...
for teacher_id, foerderung in teachers_data:
    if foerderung == 1:
        # Schule ist gefördert -> Lehrer bekommt zufällig Datum oder NULL
        if random.choice([True, False]):  # 50% Wahrscheinlichkeit
...
print(f"Mit Infowebinar-Datum: {with_datetime}")
print(f"Mit NULL: {with_null}")
