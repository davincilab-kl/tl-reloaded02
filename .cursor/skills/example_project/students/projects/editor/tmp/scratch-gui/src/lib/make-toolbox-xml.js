 ...
 * @param {?object} editorConfig - Configuration object to hide specific block categories.
 * @returns {string} - a ScratchBlocks-style XML document for the contents of the toolbox.
 */
const makeToolboxXML = function (isInitialSetup, isStage = true, targetId, categoriesXML = [],
    costumeName = '', backdropName = '', soundName = '', colors = defaultColors, editorConfig = {}) {
    ...
    const motionXML = editorConfig.hideCategoryMotion ? null : (moveCategory('motion') || motion(isInitialSetup, isStage, targetId, colors.motion));
    const looksXML = editorConfig.hideCategoryLooks ? null : (moveCategory('looks') ||
        looks(isInitialSetup, isStage, targetId, costumeName, backdropName, colors.looks));
    const soundXML = editorConfig.hideCategorySound ? null : (moveCategory('sound') || sound(isInitialSetup, isStage, targetId, soundName, colors.sounds));
    const eventsXML = editorConfig.hideCategoryEvent ? null : (moveCategory('event') || events(isInitialSetup, isStage, targetId, colors.event));
    const controlXML = editorConfig.hideCategoryControl ? null : (moveCategory('control') || control(isInitialSetup, isStage, targetId, colors.control));
    const sensingXML = editorConfig.hideCategorySensing ? null : (moveCategory('sensing') || sensing(isInitialSetup, isStage, targetId, colors.sensing));
    const operatorsXML = editorConfig.hideCategoryOperators ? null : (moveCategory('operators') || operators(isInitialSetup, isStage, targetId, colors.operators));
    const variablesXML = editorConfig.hideCategoryData ? null : (moveCategory('data') || variables(isInitialSetup, isStage, targetId, colors.data));
    const myBlocksXML = editorConfig.hideCategoryProcedures ? null : (moveCategory('procedures') || myBlocks(isInitialSetup, isStage, targetId, colors.more));

    const everything = [
        xmlOpen
    ];
    
    // Füge Kategorien nur hinzu, wenn sie nicht ausgeblendet sind
    if (motionXML) {
