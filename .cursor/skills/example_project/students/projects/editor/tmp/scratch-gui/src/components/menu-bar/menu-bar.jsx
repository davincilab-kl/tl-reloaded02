...
import LoadButton from './load-button.jsx';
...
class MenuBar extends React.Component {
    ...
    handleClickSave () {
        // Speichere Projekt über postMessage an Parent-Frame
        if (this.props.vm) {
            const projectData = this.props.vm.toJSON();
            const projectTitle = this.props.projectTitle || '';
            
            // Erstelle Screenshot vom Stage
            if (this.props.vm.renderer && this.props.vm.renderer.requestSnapshot) {
                try {
                    // Erstelle Screenshot asynchron
                    this.props.vm.postIOData('video', {forceTransparentPreview: true});
                    this.props.vm.renderer.requestSnapshot((dataURI) => {
                        this.props.vm.postIOData('video', {forceTransparentPreview: false});
                        
                        // Sende an Parent-Frame mit Screenshot
                    ...
                    console.error('Fehler beim Erstellen des Screenshots:', e);
                    // Sende ohne Screenshot
                    if (window.parent && window.parent !== window) {
                        window.parent.postMessage({
                            type: 'saveProject',
                            projectData: projectData,
                ...
                // Kein Renderer verfügbar, sende ohne Screenshot
                if (window.parent && window.parent !== window) {
                    window.parent.postMessage({
                        type: 'saveProject',
                        projectData: projectData,
    ...
    handleClickLoad () {
        // Sende postMessage an Parent-Frame zum Öffnen des Load-Modals
        if (window.parent && window.parent !== window) {
            window.parent.postMessage({
                type: 'openLoadProjectModal'
            }, '*');
        }
    }
    handleClickPublish () {
        // Sende postMessage an Parent-Frame zum Öffnen des Publish-Modals
        if (window.parent && window.parent !== window) {
            window.parent.postMessage({
                type: 'openPublishProjectModal'
            }, '*');
        }
    }
    handleClickBack () {
        ...
        if (window.parent && window.parent !== window) {
            // Sende postMessage an Parent-Frame zum Navigieren
            window.parent.postMessage({
                type: 'navigateToProjects'
            }, '*');
        } else {
            // Navigiere direkt zur Projekt-Übersichtsseite
        ...
        return (
            ...
            >
                <div className={styles.mainMenu}>
                    ...
                    <div className={classNames(styles.menuBarItem)}>
                        {/* Speichern, Laden und Veröffentlichen Buttons */}
                        {!this.props.editorConfig?.hideSave && (
                            <SaveButton
                                className={styles.menuBarButton}
                                onClick={this.handleClickSave}
                                isSuccess={this.props.saveStatus?.isSuccess}
                                successMessage={this.props.saveStatus?.message}
                            />
                        )}
                        {!this.props.editorConfig?.hideLoad && (
                            <LoadButton
                                className={styles.menuBarButton}
                                onClick={this.handleClickLoad}
                            />
                        )}
                        {!this.props.editorConfig?.hidePublish && (
                            this.props.canShare ? (
                                    ...
                                    <ProjectWatcher onDoneUpdating={this.props.onSeeCommunity}>
                                        {
                                            waitForUpdate => (
                                                <PublishButton
                                                    className={styles.menuBarButton}
                                                    isPublished={this.props.isShared}
                    ...
                    <div className={classNames(styles.menuBarItem, styles.backButtonWrapper)}>
                        <Button
                            className={classNames(styles.menuBarButton, styles.backButton)}
                    ...
                    {/* Tutorials-Button und Debug-Button entfernt für lokale Installation */}
                    {/* <div className={styles.fileGroup}>
                        <div
                ...
                {/* Account-Info-Group entfernt für lokale Installation */}
...
const mapStateToProps = (state, ownProps) => {
    ...
    return {
        ...
        saveStatus: state.scratchGui.saveStatus,
