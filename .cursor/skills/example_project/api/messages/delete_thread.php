<?php
    ...
    try {
        // Prüfe ob archived Spalte existiert, falls nicht erstelle sie
        ...
        // Alle Nachrichten des Threads archivieren statt löschen
        $sql = "UPDATE messages SET archived = 1 WHERE thread_id = ? OR id = ?";
        ...
        $archived_rows = $stmt->affected_rows;
        $stmt->close();

        echo json_encode([
            'success' => true,
            'thread_id' => $thread_id,
            'archived_messages' => $archived_rows,
            'message' => 'Thread erfolgreich ins Archiv verschoben'
        ]);
