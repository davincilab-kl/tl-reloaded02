<?php
    ...
    try {
        ...
        $check_sql = "SELECT p.id, p.student_id 
                     FROM projects p 
        ...
        try {
            ...
            error_log("[submit_project.php] Fehler beim Aktualisieren der Projektanzahl: " . $count_error->getMessage());
        }

        // Prüfe und aktualisiere Lehrer-Status nach Projekt-Einreichung (erstes eingereichtes Projekt)
            ...
            if ($teacher_stmt->execute()) {
                ...
                if ($teacher_row = $teacher_result->fetch_assoc()) {
                    ...
                    try {
                        ...
                        // Nicht kritisch - nur loggen
                        error_log("[submit_project.php] Fehler bei Status-Prüfung: " . $status_error->getMessage());
                    }
                }
            }
            $teacher_stmt->close();
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Projekt erfolgreich eingereicht'
        ]);

    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['success' => false, 'error' => 'Database error: ' . $e->getMessage()]);
        error_log("[submit_project.php] Fehler: " . $e->getMessage());
    }
