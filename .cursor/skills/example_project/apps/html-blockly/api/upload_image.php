<?php
    ...
    // Eindeutigen Dateinamen generieren (einfache Nummer)
    $extension = pathinfo($file['name'], PATHINFO_EXTENSION);
    
    // Finde die nächste verfügbare Nummer
    $counter = 1;
    do {
        $filename = 'img_' . $counter . '.' . $extension;
        $filepath = $uploadDir . $filename;
        $counter++;
        // Sicherheitscheck: Maximal 10000 Bilder
        if ($counter > 10000) {
            http_response_code(500);
            echo json_encode(['success' => false, 'error' => 'Zu viele Bilder']);
            exit;
        }
    ...
    $url = $baseUrl . '/apps/html-blockly/api/uploads/index.php?file=' . urlencode($filename);
